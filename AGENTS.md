...
- `node scripts/verify-hotel-nearby-counts.mjs`: verify each accommodation has at least 100 usable `hotel_nearby` restaurant/cafe candidates; add `--list` to audit counted IDs or `--json` for machine-readable output.
