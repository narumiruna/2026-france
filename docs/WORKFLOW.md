...
- 每個住宿地點附近至少需要 `100` 間可用餐廳或咖啡廳候選；計數只包含 `hotel_nearby: true`、非歇業、且可旅中使用的項目。
...
- JSON 欄位需符合設計文件契約。  
- `lat/lng` 必須可解析為數值。  
- 住宿附近覆蓋數需跑過 `node scripts/verify-hotel-nearby-counts.mjs`；`data/cities/*.json` 與 `data/restaurants.json` 四個住宿點都達 `100/100` 後，才可把住宿周邊研究標記為完成。
- 分頁驗證四項必過：
